...
//  OpenLib
//
//  Created by xgj on 14-2-13.
//  Copyright (c) 2014年 xgj. All rights reserved.
//
...
static const NSTimeInterval CSToastDefaultDuration  = 2.f;
...
// activity
static const CGFloat CSToastActivityWidth       = 50.f;
static const CGFloat CSToastActivityHeight      = 20.f;
...
static const NSInteger CSToastViewTag         =       123411231;
...
- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position {
    UIView *toast = [self viewWithTag:CSToastViewTag];
    if (!toast) {
...
- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position title:(NSString *)title {
    UIView *toast = [self viewWithTag:CSToastViewTag];
    if (!toast) {
        toast = [self viewForMessage:message title:title image:nil];
    }
    [self showToast:toast duration:duration position:position];
}
...
- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position image:(UIImage *)image {
    UIView *toast = [self viewWithTag:CSToastViewTag];
    if (!toast) {
        toast = [self viewForMessage:message title:nil image:image];
    }
    [self showToast:toast duration:duration position:position];
}
...
- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration  position:(id)position title:(NSString *)title image:(UIImage *)image {
    UIView *toast = [self viewWithTag:CSToastViewTag];
    if (!toast) {
        toast = [self viewForMessage:message title:title image:image];
    }
    [self showToast:toast duration:duration position:position];
}
...
- (void)makeToastActivity:(id)position {
    ...
    ActivityView *activityView = [[ActivityView alloc] initWithFrame:CGRectMake(0, 0, CSToastActivityWidth, CSToastActivityHeight)];
